...
import pdb
sys.path.append('../')
from model.Cifar.ResNet.resnet import *

url = 'cfcui/NIPS/ASNet/model/Cifar/ResNet/'
...
def main():
    ...
    model = resnet110(num_class=100)#torch.nn.DataParallel(resnet110())
    model.to(device)
...
def save_checkpoint(state, is_best, filename=url+'checkpoint.pth.tar'):
    """
...
def accuracy(output, target, topk=(1,)):
    ...
    pred = pred.t()
    correct = pred.eq(target.to(device).view(1, -1).expand_as(pred.to(device)))
