...
# class ASNET_TT(nn.Module):
#     def __init__(self, pre_model,TT_AS_model,basis_layer,PCE_coeff=None,device='cuda',num_classes=10):
#         super(ASNET_TT, self).__init__() 
#         self.premodel = copy.deepcopy(pre_model) 
#         self.TTAS = TT_AS_model.to(device)
# #         if TT_AS_model.weight is not None:
# #             self.TTAS.factors.data = copy.deepcopy(TT_AS_model.V) 
#         if basis_layer is None:
#             raise ValueError('basis_layer must be given')
#         self.PCE_basis =  basis_layer
#         self.PCE = nn.Linear(basis_layer.nbasis, num_classes, bias=False).to(device)
#         if PCE_coeff is not None:     
#             self.PCE.weight.data = copy.deepcopy(PCE_coeff).t()    
#     def forward(self,x):
#         x = self.premodel(x)
#         x = x.view([x.shape[0],*self.TTAS.in_size])
#         x = self.TTAS(x) 
#         x = self.PCE_basis(x)
#         x = self.PCE(x)
...
# class ASNET_dropout(nn.Module):
#     def __init__(self, pre_model,AS_model, 
#                  basis_layer,PCE_coeff,device='cuda',num_classes=10):
#         super(ASNET_dropout, self).__init__()
#         self.premodel = copy.deepcopy(pre_model)
#         self.AS = nn.Linear(AS_model.V.shape[0],AS_model.V.shape[1],  bias=False)
#         self.PCE_basis = basis_layer
#         self.PCE = nn.Linear(basis_layer.nbasis,num_classes, bias=False)
